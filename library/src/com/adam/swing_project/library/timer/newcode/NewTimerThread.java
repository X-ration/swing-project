...
public class NewTimerThread extends Thread{
...
//                this.startMills += startDelayUnit.toMillis(startDelayDuration);
//            }
    ...
    @Override
    public void run() {
        logger.logInfo("NewTimerThread started");
...
//                logger.logDebug("Jump out of wait, terminateSign=" + terminateSign);
        ...
        logger.logInfo("NewTimerThread stopped");
    }
    ...
    public static void main(String[] args) {
        NewTimerThread newTimerThread = new NewTimerThread();
        newTimerThread.start();
        Random random = new Random();
        int i=0;
        Runtime.getRuntime().addShutdownHook(new Thread(()->{
            newTimerThread.terminate();
            try {
                newTimerThread.join();
            } catch (InterruptedException e) {
                e.printStackTrace();
        ...
        while(true) {
            ...
            i++;
            TimerTask task = newTimerThread.new FixedDelayTimerTask(3, 1, TimeUnit.SECONDS);
            task.taskName = "Task " + i;
            task.setAction(()-> System.out.println(task.taskName + " action " + System.currentTimeMillis()));
            newTimerThread.addTask(task);
        }
    }
